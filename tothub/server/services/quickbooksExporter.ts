...
export class QuickBooksExporter {
  ...
  static async generatePayrollCSV(payPeriodId: string): Promise<string> {
    try {
      ...
      // Generate CSV rows
      const rows = payrollData.map(({ payStub, staff: staffMember, payPeriod }: any) => [
  ...
  static async generatePayrollIIF(payPeriodId: string): Promise<string> {
    try {
      ...
      payrollData.forEach(({ payStub, staff: staffMember }: any, index: number) => {
  ...
  static async generateGeneralLedgerSummary(payPeriodId: string): Promise<{
    ...
    try {
      ...
      const totals = payrollData.reduce((acc: any, { payStub }: any) => {
  ...
  static async generateTaxReports(quarter: number, year: number): Promise<{
    ...
    try {
      ...
      const quarterTotals = quarterPayroll.reduce((acc: any, { payStub }: any) => {
      ...
      return {
        form941Data: {
          quarter,
          year,
          employeeCount: new Set(quarterPayroll.map((p: any) => p.staff.id)).size,
          ...quarterTotals,
