...
export class DocumentExpirationService {
  ...
  static async getExpiringDocuments(daysAhead: number = 30) {
    ...
    return await db
      .select({
        id: documents.id,
        title: documents.title,
        expirationDate: documents.expiresAt,
        status: documents.status,
        documentNumber: documents.documentNumber,
        issuingAuthority: documents.issuingAuthority,
        contactInfo: documents.contactInfo,
      ...
      .where(and(
        eq(documents.isActive, 1),
        lte(documents.expirationDate, futureDate.toISOString()),
        gte(documents.expirationDate, new Date().toISOString())
      ))
      .orderBy(asc(documents.expirationDate));
  }
  ...
  static async getPendingReminders() {
    ...
    return await db
      ...
      .where(and(
        ...
        eq(documents.isActive, 1),
        gte(documentReminders.reminderDate, today.toISOString()),
        lte(documentReminders.reminderDate, tomorrow.toISOString()),
  ...
  static async getStatistics() {
    ...
    return {
      ...
      categoryBreakdown: byCategory.reduce((acc: Record<string, number>, item: any) => {
        if (item.category) {
