...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    // Create session tracking (best-effort in dev)
    let sessionId = null;
    try {
      ...
      console.warn('Session persistence failed (continuing without DB session):', e);
      // Create a simple session ID for express-session
...
router.post('/parent/login', async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      console.warn('Parent session persistence failed (continuing without DB session):', e);
      // Create a simple session ID for express-session
      sessionId = `temp_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
      req.session!.sessionId = sessionId;
      req.session!.userId = parent.id;
