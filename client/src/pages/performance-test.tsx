...
export default function PerformanceTest() {
  ...
  const seedTestDataMutation = useMutation({
    mutationFn: () => apiRequest("POST", "/api/test/seed-data", {}),
    onMutate: () => {
    ...
    onSuccess: (data) => {
      ...
      setPerformanceMetrics(prev => ({
        ...prev,
        seedTime: Date.now() - prev.startTime
      }));
  ...
  const clearTestDataMutation = useMutation({
    mutationFn: () => apiRequest("DELETE", "/api/test/clear-data", {}),
    onSuccess: () => {
      setTestResults(null);
  ...
  const getComplianceStatus = () => {
    const nonCompliantRooms = (ratios as any[]).filter(ratio => !ratio.isCompliant);
    return {
      compliant: (ratios as any[]).length - nonCompliantRooms.length,
      nonCompliant: nonCompliantRooms.length,
      total: (ratios as any[]).length
    };
  };
              ...
              {(ratios as any[]).length === 0 ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {(ratios as any[]).map((ratio: any, index: number) => (
