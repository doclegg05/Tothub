...
router.post('/mfa/setup', mfaRateLimit, createAuditMiddleware('mfa_setup'), async (req, res) => {
  try {
    ...
    AuditService.logAuth(userId, 'LOGIN', true, req.ip, req.headers['user-agent'] || '');
...
router.post('/mfa/verify', mfaRateLimit, createAuditMiddleware('mfa_verify'), async (req, res) => {
  try {
    ...
    if (isValid) {
      ...
      AuditService.logAuth(userId, 'FAILED_LOGIN', false, req.ip, req.headers['user-agent'] || '');
...
router.post('/compliance/delete-request', authRateLimit, createAuditMiddleware('data_deletion_request'), async (req, res) => {
  try {
    ...
    AuditService.logCompliance(userId, 'data_deletion_request', dataType, true, req.ip, req.headers['user-agent'] || '', { reason });
...
router.post('/compliance/export-request', authRateLimit, createAuditMiddleware('data_export_request'), async (req, res) => {
  try {
    ...
    AuditService.logCompliance(userId, 'data_export_request', exportableTypes.join(','), true, req.ip, req.headers['user-agent'] || '');
