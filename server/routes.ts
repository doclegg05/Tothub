...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post(
    ...
    async (_req: Request, res: Response) => {
      try {
        ...
        res
          .status(500)
          .json({ ok: false, message: "Failed to optimize memory" });
      }
    }
  );

  // Cache stats endpoint
  app.get("/api/_infra/cache", auth, async (_req: Request, res: Response) => {
    try {
  ...
  app.get("/api/_infra/metrics", auth, async (_req: Request, res: Response) => {
    try {
      const service = MonitoringService.getInstance();
      res.json({ ok: true, metrics: service.getCurrentMetrics() });
    } catch (e) {
      res.status(500).json({ ok: false, message: "Failed to fetch metrics" });
    }
  });
    ...
    try {
      ...
      try {
        ...
        if (
          ...
          e.message.includes("UNIQUE constraint failed: staff.email")
        ) {
          return res
            .status(400)
  ...
  app.get("/api/schedules", auth, async (req: Request, res: Response) => {
    try {
      ...
      let schedules;
      if (date) {
        schedules = await storage.getStaffSchedulesByDate(
      ...
      if (error instanceof Error) {
        ...
        if (
          errorMessage.includes(
            "Cannot schedule staff for a time in the past"
          ) ||
  ...
  app.post(
    "/api/schedules/:id/present",
    auth,
    async (req: Request, res: Response) => {
      try {
        const { actualStart } = req.body;
        const updatedSchedule = await storage.markStaffPresent(
          req.params.id,
  ...
  app.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const updatedSchedule = await storage.markStaffEnd(
          req.params.id,
          new Date(actualEnd)
        );
        res.json(updatedSchedule);
